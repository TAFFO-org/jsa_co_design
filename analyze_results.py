...
def main(argv):
    ...
    for index, config in configs.iterrows():
        ...
        compile_time_path = f"{job_file_base}.compile_time.csv"
        ...
        compile_time = 0
        try:
            compile_breakdown = retriveCompilationTime(compile_time_path)
            # ret = {
            #     # "total_time": compile_breakdown.iloc[0]['taffo_end'] - compile_breakdown.iloc[0]['taffo_start'],
            #     "TAFFO C to LLVM IR": compile_breakdown.iloc[0]['init_start'] - compile_breakdown.iloc[0]['taffo_start'],
            #     "TAFFO LLVM IR to binary": compile_breakdown.iloc[0]['taffo_end'] - compile_breakdown.iloc[0]['backend_start'],
            #     "TAFFO initializer": compile_breakdown.iloc[0]['vra_start'] - compile_breakdown.iloc[0]['init_start'],
            #     "TAFFO VRA": compile_breakdown.iloc[0]['dta_start'] - compile_breakdown.iloc[0]['vra_start'],
            #     "TAFFO DTA": compile_breakdown.iloc[0]['conversion_start'] - compile_breakdown.iloc[0]['dta_start'],
            #     "TAFFO conversion": compile_breakdown.iloc[0]['backend_start'] - compile_breakdown.iloc[0]['conversion_start'],
            # }
            compile_time = compile_breakdown.iloc[0]['taffo_end'] - compile_breakdown.iloc[0]['taffo_start']
        except:
            print(f"{bench}_{scaling} compilation time is not available")
            compile_time = np.nan
        ...
        kernel_float_op_count = 0
        kernel_by_opcode_count = {
            'fmul': 0,
            'fdiv': 0,
            'fadd': 0,
            'fsub': 0,
        ...
        with open(trace_path) as f:
            kernel_segment = False
            for line in f:
                line = line.strip()
                parts = line.split(" ")
                if len(parts) == 0: continue
                if parts[0] == "TRACE_TIMER":
                    timer_name = parts[1]
                    if timer_name == "timer_start":
                        kernel_segment = True
                    else:
                        kernel_segment = False
                if parts[0] == "TAFFO_TRACE":
                    ...
                    if opcode == 'fmul' or opcode == 'fdiv' or opcode == 'fadd' or opcode == 'fsub':
                        ...
                        if kernel_segment:
                            kernel_float_op_count += 1
                            kernel_by_opcode_count[opcode] += 1
        ...
        try:
            ...
            stats_row = {
                ...
                'dyn_fdiv': by_opcode_count['fdiv'],
                'dyn_fmul': by_opcode_count['fmul'],
                'dyn_fadd': by_opcode_count['fadd'],
                'dyn_fsub': by_opcode_count['fsub'],
                'kernel_dyn_fdiv': kernel_by_opcode_count['fdiv'],
                'kernel_dyn_fmul': kernel_by_opcode_count['fmul'],
                'kernel_dyn_fadd': kernel_by_opcode_count['fadd'],
                'kernel_dyn_fsub': kernel_by_opcode_count['fsub'],
                ...
                'kernel_float_op_count': kernel_float_op_count,
            ...
            logging.exception(exception1)
            print(f"could not handle row: {str(exception1)}")
    ...
    scale_1_summary = (
        stats_summary[(stats_summary['scale'] == 1) &
                      (stats_summary['mode'] == 'float') &
                      (stats_summary['float_type'] == 'float32')
        ]
    )[['bench', 'float_op_count', 'kernel_float_op_count']]
    scale_1_summary.rename(columns={
        'float_op_count': 'float_op_count_default',
        'kernel_float_op_count': 'kernel_float_op_count_default'},
        inplace=True
    )
    stats_summary = stats_summary.merge(scale_1_summary, on='bench', how='left')
    print(stats_summary)
...
def plot_error(results_path, stats_summary_full):
    ...
    fig.suptitle('Calculated and predicted log10(relative error)')
...
def retriveCompilationTime(path):
    content = pd.read_csv(path, sep=',').mul(1000)
    return content

def ReadValues(filename):
    with open(filename, 'r') as f:
        l = f.readline()
        if l.startswith('Floating point exception'):
            yield np.nan
            return
        while l != '':
