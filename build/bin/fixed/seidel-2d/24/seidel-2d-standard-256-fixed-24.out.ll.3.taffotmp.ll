; ModuleID = './build/bin/fixed/seidel-2d/24/seidel-2d-standard-256-fixed-24.out.ll.2.taffotmp.ll'
...
@.str = private unnamed_addr constant [50 x i8] c"target('A') scalar(range(25.6,5132.799805) final)\00", section "llvm.metadata", !taffo.info !0
...
entry:
  ...
  call void @scale_2d.3(i32 noundef 20, i32 noundef 20, float* noundef %0, i32 noundef 256), !taffo.initweight !55, !taffo.target !48, !taffo.info !49, !taffo.originalCall !56, !taffo.constinfo !57
...
; Function Attrs: noinline nounwind uwtable
define internal void @init_array.2(i32 noundef %n, [20 x float]* noundef %A) #0 !taffo.initweight !123 !taffo.funinfo !124 !taffo.sourceFunction !53 {
entry:
  br label %for.cond

for.cond:                                         ; preds = %for.inc10, %entry
  %i.0 = phi i32 [ 0, %entry ], [ %inc11, %for.inc10 ], !taffo.initweight !47, !taffo.info !75
  %cmp = icmp slt i32 %i.0, %n, !taffo.initweight !52, !taffo.info !125
  br i1 %cmp, label %for.body, label %for.end12, !taffo.initweight !55, !taffo.info !75
...
for.cond3:                                        ; preds = %for.inc, %for.body
  %j.0 = phi i32 [ 0, %for.body ], [ %inc, %for.inc ], !taffo.initweight !47, !taffo.info !75
  %cmp4 = icmp slt i32 %j.0, %n, !taffo.initweight !52, !taffo.info !125
  br i1 %cmp4, label %for.body5, label %for.end, !taffo.initweight !55, !taffo.info !75

for.body5:                                        ; preds = %for.cond3
  %conv = sitofp i32 %i.0 to float, !taffo.initweight !52, !taffo.info !75
  %add = add nsw i32 %j.0, 2, !taffo.initweight !52, !taffo.info !126, !taffo.constinfo !27
  %conv6 = sitofp i32 %add to float, !taffo.initweight !55, !taffo.info !126
  %mul = fmul float %conv, %conv6, !taffo.initweight !55, !taffo.info !128
  %add7 = fadd float %mul, 2.000000e+00, !taffo.initweight !67, !taffo.info !130, !taffo.constinfo !77
  %conv8 = sitofp i32 %n to float, !taffo.info !90
  %div = fdiv float %add7, %conv8, !taffo.initweight !78, !taffo.info !132
  %arrayidx = getelementptr inbounds [20 x float], [20 x float]* %A, i32 %i.0, !taffo.initweight !52, !taffo.info !134
  %arrayidx9 = getelementptr inbounds [20 x float], [20 x float]* %arrayidx, i32 0, i32 %j.0, !taffo.initweight !52, !taffo.info !134
  store float %div, float* %arrayidx9, align 4, !taffo.initweight !55, !taffo.info !79
  br label %for.inc

for.inc:                                          ; preds = %for.body5
  %inc = add nsw i32 %j.0, 1, !taffo.initweight !52, !taffo.info !135, !taffo.constinfo !27
...
for.inc10:                                        ; preds = %for.end
  %inc11 = add nsw i32 %i.0, 1, !taffo.initweight !52, !taffo.info !135, !taffo.constinfo !27
...
; Function Attrs: noinline nounwind uwtable
define internal void @scale_2d.3(i32 noundef %n, i32 noundef %m, float* noundef %val, i32 noundef %factor) #0 !taffo.initweight !139 !taffo.funinfo !140 !taffo.sourceFunction !56 {
entry:
  br label %for.cond
...
for.body3:                                        ; preds = %for.cond1
  ...
  %mul = fmul float %1, %conv, !taffo.initweight !143, !taffo.info !144
...
!5 = !{double 0.000000e+00, double 0x40B40CCCC0053E2D}
...
!50 = !{double 2.560000e+01, double 0x40B40CCCC0053E2D}
!51 = !{i32 1}
!52 = !{i32 2}
...
!108 = !{double 5.120000e+01, double 0x40C40CCCC0053E2D}
!109 = !{i1 false, !110, i1 false, i2 -1}
!110 = !{double 0x4053333333333334, double 0x40CE13332007DD44}
!111 = !{i1 false, !112, i1 false, i2 1}
!112 = !{double 1.024000e+02, double 0x40D40CCCC0053E2D}
!113 = !{i1 false, !112, i1 false, i2 -1}
!114 = !{i1 false, !115, i1 false, i2 1}
!115 = !{double 1.280000e+02, double 0x40D90FFFF0068DB8}
!116 = !{i1 false, !117, i1 false, i2 1}
!117 = !{double 2.304000e+02, double 0x40E68E665805E5F2}
!118 = !{i1 false, !119, i1 false, i2 1}
!119 = !{double 2.560000e+01, double 0x40B40CCCC0053E2C}
!120 = distinct !{!120, !29}
!121 = distinct !{!121, !29}
!122 = distinct !{!122, !29}
!123 = !{i32 -1, i32 2}
!124 = !{i32 1, !90, i32 1, !49}
!125 = !{i1 false, !64, i1 false, i2 -1}
!126 = !{i1 false, !127, i1 false, i2 -1}
!127 = !{double -1.800000e+01, double 2.200000e+01}
!128 = !{i1 false, !129, i1 false, i2 -1}
!129 = !{double -4.400000e+02, double 4.400000e+02}
!130 = !{i1 false, !131, i1 false, i2 -1}
!131 = !{double -4.380000e+02, double 4.420000e+02}
